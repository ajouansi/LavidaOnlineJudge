<?
        ...
        if( isset($cid) && isset($pid) ) {
        	...
        	$pn=$_GET['pn'];
        	$contest = 0;
        	$sql = "select contest_id from contest_problem where problem_id='$pn'";
        	$tmp = @mysql_query($sql);
  			$now = time();
        	while( $row = @mysql_fetch_row($tmp) ) {
        		$sql = "select start_time from contest where contest_id='$row[0]'";
        		$tmp2 = @mysql_query($sql);
        		while( $ret = @mysql_fetch_row($tmp2) ) {
	        		$ti = strtotime($ret[0]);
	        		if( $now < $ti ) {
	        				$contest = 1;
	        				break;
        ...
        if( $contest == 1 && !isset($_SESSION['administrator']) ) {
        		$sql = "select contest_id from contest_problem where problem_id='$pn'";
        		$tmp = @mysql_query($sql);
        		?>
<div class="row-fluid">
        <div class="span12">
        		<table class="table">
        		<thead>
					This problem is used by under contest(s).
        		</thead>
        		<tbody>
        		<?
        		while( $row = @mysql_fetch_row($tmp) ) {
        			$sql = "select * from contest where contest_id='$row[0]'";
        			$tmp2 = @mysql_query($sql);
        			while( $ret = @mysql_fetch_row($tmp2) ){
	        		?>
						<tr><td><a href="/contest/<?=$ret[0]?>"><?=$ret[1]?></a></td></tr>
	       			<?
        			}
        		}
        		?>
